...
class JsonFormatter(logging.Formatter):
    ...
    def formatMessage(self, record) -> dict:
        ...
        """
        return {fmt_key: record.__dict__[fmt_val]
    ...
    def format(self, record) -> str:
        ...
        return json.dumps(message_dict, default=str, ensure_ascii=False, indent=3)


def create_logger(logger_name, level=LOGGING_LEVEL) -> logging.Logger:
    # get named logger
    ...
    # create handler with TimedRotatingFileHandler instead of Filehandler
    handler = TimedRotatingFileHandler(
        filename=os.path.join("logs", 'runtime.log'), when='D', interval=1,
        backupCount=7, encoding='utf-8', delay=False
    )

    # create formatter based on overwritten class and add to handler
